...
def bev_from_pcl(lidar_pcl, configs,cnt_frame):
    ...
    #intensity_unique, counts = np.unique(lidar_pcl_cpy[:,0:2], axis= 1, return_index=1)
    _,intensity_unique,counts = np.unique(lidar_pcl_cpy[:,0:2], axis= 0, return_index=1, return_counts=True)
    lidar_pcl_top = lidar_pcl_cpy[intensity_unique]
